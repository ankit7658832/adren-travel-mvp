...
There is currently no shared component/design-system layer (`src/shared/components/` is empty) and no CSS framework/methodology chosen (`search-dashboard` uses zero `className`s, plain semantic HTML plus one global stylesheet). This is a real decision point, not an oversight to silently work around — the second feature that needs styled UI (buttons, cards, form fields, the map+results split-panel layout in PRD §21.1) is the one that will either establish a real design-system convention or start an inconsistent pile of one-off styles per feature. Recommendation, to be confirmed with whoever owns visual design before the first styled feature ships:

- Pick **one** styling approach platform-wide (Tailwind is a reasonable default given no CSS-in-JS runtime cost and pairs well with a headless component approach; CSS Modules is the other defensible choice given zero dependencies exist yet) — the point is picking one, not which one, and writing it into `frontend-react-vite`'s feature-folder shape once chosen.
- Build `src/shared/components/` as genuinely generic primitives (`Button`, `Card`, `TextField`, `Select`, a `MapPanel`/`ResultsPanel` layout pair for the search+itinerary screens which share that split-panel shape per PRD §21.1/§21.2) — a component belongs in `shared/components` only if at least two features use it or clearly will; don't pre-build a component library speculatively ahead of real feature needs.
- **White-label theming (PRD §13.2 — logo, background, primary/secondary color per Consultant) is a design-system-level concern, not a per-component one.** Whatever styling approach is chosen must support runtime-configurable theme tokens (CSS custom properties are the natural mechanism regardless of Tailwind/CSS-Modules choice, since both can consume `var(--adren-primary)`-style tokens), because Consultant branding is applied per-tenant at runtime, not at build time — a design system built around build-time-only theme values (e.g., Tailwind config-baked colors with no runtime override path) would need a rework the moment white-label theming is implemented for real, so account for it now even though `whitelabel` is currently just a backend package-info stub.
- Accessibility-baseline components (§`RULES.md` §7.3) should be correct *once*, in `shared/components`, rather than re-solved per feature — a shared `TextField` that always wires `label`+`htmlFor`+`aria-invalid`/`aria-describedby` correctly means every consumer gets that for free, versus every feature author re-deriving the a11y wiring `search-dashboard` currently does by hand at the JSX level.
