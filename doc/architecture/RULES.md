...
**Fixed (TST-03):** the frontend's `apiClient.ts` had zero request interceptor attaching the auth token to any outgoing call, despite `AuthSessionContext` already reading one from `localStorage` for client-side role-gating — every real API call from the running app 401'd against the backend's `.anyRequest().authenticated()` policy above. Discovered while building a Playwright e2e spec that finally logs in for real (via `DevAuthController`'s `SPRING_PROFILES_ACTIVE=dev`-only token-minting endpoint) instead of navigating straight past auth like every prior e2e spec's own documented gap. Fixed with a request interceptor on `apiClient` reading `AUTH_TOKEN_STORAGE_KEY` and attaching `Authorization: Bearer <token>` when present — confirmed with a direct `curl` comparison (no token → 401; a real dev token → past auth, a plain 400 validation error) before and after the fix, and all 10 e2e specs (previously only ever passing 0-4 of them for real) now pass.
