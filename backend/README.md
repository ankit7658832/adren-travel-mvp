...
| `payments` | Payments, Yield/Markup & Wallet (Section 12) | Per-Consultant, per-category markup configuration (FIN-01), wallet balance/credit-limit/pending-holds query with lazy auto-provisioning (FIN-06), Adren commission calculated separately from Consultant markup (FIN-02), and the currency buffer applied before markup (FIN-03) implemented; Stripe/holds-ledger follow |
