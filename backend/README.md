...
|---|---|---|
| `booking` | Core Booking Engine (Section 9) | Reference implementation; Hotel line items (BOK-03) priced through the `payments` module's FIN-05 pipeline; saving as a Quotation now requires a line item and creates a `Quotation` row with a `validUntil` window, and becomes read-only to further line items (BOK-08/BOK-09); Stripe-webhook-gated booking confirmation (FIN-11) |
